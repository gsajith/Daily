 ...
 * creation.
 * <p/>
 * An instance of this class is meant to be passed to the method
...
public class DragSortController extends SimpleFloatViewManager implements View.OnTouchListener, GestureDetector.OnGestureListener {
  ...
  private float mFlingSpeed = 5000f;
  ...
  private GestureDetector.OnGestureListener mFlingRemoveListener =
    new GestureDetector.SimpleOnGestureListener() {
      ...
      public final boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX,
        ...
        Log.d("ALPHA", "fling called " + mRemoveEnabled + " " + mIsRemoving);
        if (mRemoveEnabled && mIsRemoving) {
          int w = mDslv.getWidth();
          int minPos = w / 5;
          Log.d("ALPHA", velocityX + " " + mFlingSpeed + " " + mPositionX + " " + minPos);
  ...
  public void setRemoveEnabled(boolean enabled) {
    Log.d("ALPHA", "remove enabled: " + enabled);
    mRemoveEnabled = enabled;
    if (mDslv != null) {
      mDslv.setRemoveEnabled(mRemoveEnabled);
    }
  }
  ...
  public boolean startDrag(int position, int deltaX, int deltaY) {
    ...
    /*
    REMOVE THESE TO RE-ENABLE DRAGS IN SET DIRECTIONS
     */
    dragFlags = 0;
    dragFlags |= DragSortListView.DRAG_POS_Y;
    dragFlags |= DragSortListView.DRAG_NEG_X;
    dragFlags |= DragSortListView.DRAG_NEG_Y;
    dragFlags |= DragSortListView.DRAG_POS_X;
    ...
    switch (action) {
      ...
      case MotionEvent.ACTION_UP:
        if (mRemoveEnabled && mIsRemoving) {
          ...
          if (x > removePoint) {
            //REMOVE THIS IF YOU WANT SLIDE-REMOVE AT HALFWAY BACK IN
            //TODO
  ...
  public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
    ...
    } else if (mCanDrag && (mHitPos != MISS || mFlingHitPos != MISS)) {
      mIsRemoving = true;
    }
    // return whatever
    return false;
  ...
  public final boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
    mFlingRemoveListener.onFling(e1, e2, velocityX, velocityY);
    return false;
  }
